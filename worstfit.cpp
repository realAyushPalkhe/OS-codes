...
{ 
    ...
    cout << "\nProcess No.\tProcess Size\tBlock No.\tOriginal Block Size\tRemaining Block Size\n"; 
    for (int i = 0; i < n; i++) 
    { 
        ...
        {
            cout << allocation[i] + 1 << "\t\t" 
                 << originalBlockSize[allocation[i]] << "\t\t\t" 
...
{ 
    ...
    // Create an array to store original block sizes
    int originalBlockSize[m];
    for (int i = 0; i < m; i++) {
        originalBlockSize[i] = blockSize[i]; // Store original sizes
    }
